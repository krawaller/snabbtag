export function getUrl(page, {
  ...
  favoriteTrafficOnly = this.state && this.state.favoriteTrafficOnly || this.props.favoriteTrafficOnly,
  showingDepartures = (this.state && this.state.showingDepartures || this.props.showingDepartures) || this.props.type !== 'arrivals'
} = {}) {
  const queries = [
    favorites instanceof Set && favorites.size ? `favorites=${Array.from(favorites).join(',')}` : '',
    typeof favorites === 'string' ? `favorites=${favorites}` : '',
    favoriteTrafficOnly ? `favorite_traffic_only=true` : '',
    station && (page === 'stations' || page === 'train') ? `station=${station.name || station}` : '',
    (page === 'stations' || page === 'train') && !showingDepartures && 'type=arrivals'
  ]
    .filter(Boolean)
    .join('&');

  return `${({
    station: `/stations/${station && station.name || station}/${showingDepartures ? 'departures' : 'arrivals'}`,
    stations: `/stations`,
